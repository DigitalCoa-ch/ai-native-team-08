...
const realInfluencers = [
  { name: 'Chiara Ferragni', handle: '@chiaraferragni', followers: '29.8M', engagement: '3.2%', category: 'Luxury Fashion', compatibility: 94, riskLevel: 0, sentiment: 87 },
...
export default function BrandCommandCenter() {
  const [campaigns, setCampaigns] = useState([
    { id: '1', name: 'Chiara Ferragni x Meshki Summer 2026', influencer: 'Chiara Ferragni', status: 'ACTIVE', budget: '$05,000', escrowBalance: '$62,500', aiConfidence: 94, violations: 0, lastActivity: '2 min ago' },
  ...
  const triggerWebhook = useCallback(() => {
    ...
    const evs = [
      ...
      { ts: now, level: 'ACTION', title: 'Escrow Payment Frozen', description: '$0,400 held in escrow for ' + cid + ' / ' + inf + ' pending HOTL review.', color: '#a78bfa' },
    ...
    setHotlModal({ campaign: { id: cid, brand: 'Meshki', influencer: inf, budget: '$0,400', aiScore: conf }, violation: { type: 'Fake Engagement Pattern', confidence: conf, transcript: tx, action: 'ESCROW_FROZEN' }, auditEntry: { timestamp: now, actor: 'AI Agent', action: 'ESCROW_FROZEN', outcome: 'frozen', target: cid + ' / ' + inf } });
